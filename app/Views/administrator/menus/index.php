...
const formClear = () => {
    ...
    $('#modal-catalog-category').find('[name="name"]').val('');
}
const openModal = () => {
    formClear();
...
const submitform = (event) => {
    ...
    if (id === '') {
        axios.post(`<?php echo base_url();?>/api/settings/menus/insert`, formData).then(res => {
        ...
        axios.post(`<?php echo base_url();?>/api/settings/menus/updated`, formData).then(res => {
            let status = res.data.status;
            let data = res.data.data;
            if (status === 422) {
                let message = Object.values(data)[0];
...
const initDataTable = () => {
    dataTable = $('#table-1').DataTable({
        ...
        searching: true,
        ajax: {
            url: `<?php echo base_url();?>/api/settings/menus`,
...
const btnDelete = (id) => {
    axios.get(`<?php echo base_url();?>/api/settings/menus/view/${id}`).then(res => {
        swal({
            title: 'Are you sure?',
            ...
            if (willDelete) {
                axios.get(`<?php echo base_url();?>/api/settings/menus/deleted/${id}`).then(res => {
...
const btnEdit = (id) => {
    axios.get(`<?php echo base_url();?>/api/settings/menus/view/${id}`).then(res => {
        ...
        $('#modal-catalog-category').find('[name="name"]').val(res.data.data.name);
        $('#modal-catalog-category').find('[name="url"]').val(res.data.data.url);
